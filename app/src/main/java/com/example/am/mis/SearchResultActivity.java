...
public class SearchResultActivity extends Activity {
    ...
    private void insertData(String tempName) {
        ...
        db.execSQL("insert into history(temp) values('" + tempName + "')");
        db.close();
    }
    ...
    private void queryData(String tempName) {
        Cursor cursor = helper.getWritableDatabase().rawQuery(
                "select * from history where temp like '%" + tempName + "%' order by _id desc ", null);
        // 创建adapter适配器对象
        adapter = new SimpleCursorAdapter(this, android.R.layout.simple_list_item_1, cursor, new String[] { "temp" },
    ...
    private boolean hasData(String tempName) {
        Cursor cursor = helper.getWritableDatabase().rawQuery(
                "select * from history where temp =?", new String[]{tempName});
        //判断是否有下一个
